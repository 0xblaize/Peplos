  ...
  return (
    <section className="rounded-2xl bg-peplos-ink p-4 text-white shadow-card sm:p-5">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="text-[10px] font-bold uppercase tracking-[0.16em] text-white/35">{selectedGarments.length}/2 selected</span>
      </div>
      <div className="mt-4 flex min-h-14 items-center gap-2 rounded-xl border border-white/10 bg-white/[0.05] p-2">
        {selectedGarments.length ? selectedGarments.map((item) => (
          <div key={item.id} className="group relative flex min-w-0 items-center gap-2 rounded-lg bg-white/10 py-1.5 pl-1.5 pr-2.5">
            <div className="h-9 w-8 overflow-hidden rounded-md" style={{ backgroundColor: `${item.color}55` }}>{item.image_url ? <img src={item.image_url} alt="" className="h-full w-full object-cover" /> : <span className="block h-full w-full" />}</div>
            <span className="max-w-24 truncate text-[10px] font-semibold text-white/80">{item.name}</span>
            <button type="button" onClick={() => onRemove(item)} aria-label={`Remove ${item.name}`} className="text-white/35 transition hover:text-white"><X size={12} /></button>
          </div>
